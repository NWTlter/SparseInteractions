...
parameters{
  vector[3] lambdas;   // 1: intercept, 2: slope, 3: slope2
...
model{
  ...
  for(i in 1:N){
    ...
    for(s in 1:S){alpha_eij[i,s] = exp((1-Intra[s]) * alpha_generic[1] + Intra[s] * alpha_intra[1] + (1-Intra[s]) * alpha_hat_ij[s] + 
    ((1-Intra[s]) * alpha_generic[2] + (1-Intra[s]) * alpha_hat_eij[s] + Intra[s] * alpha_intra[2]) * env[i] +
    ((1-Intra[s]) * alpha_generic[3] + (1-Intra[s]) * alpha_hat_eij[s] + Intra[s] * alpha_intra[3]) * env[i]^2
        );
    }
