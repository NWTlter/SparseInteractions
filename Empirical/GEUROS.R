...
# Set the figure name
FigName <- here("Empirical/GEUROS2018.pdf")
...
# Phosphorous
load(here("Empirical/StanFitsGEUROS_Estimate_FinalFit.rdata"))
...
for(i in 1:EnvLength){
     for(j in 1){
          SnowPost <- exp(Snow$Post$lambdas[,1] + Snow$Post$lambdas[,2] * PlotSnow[i])
...
for(i in 1:EnvLength){
     ...
     GenericPost <- exp(Snow$Post$alpha_generic[,1] + Snow$Post$alpha_generic[,2] * PlotSnow[i])
     ...
     IntraPost <- exp(Snow$Post$alpha_intra[,1] + Snow$Post$alpha_intra[,2] * PlotSnow[i])
